...
function fillFromStorage() {
	...
	var name, region, party, votes;
	for (var id = 1; id <= largestId; id ++) {
		if ("storage.candidates." + id + ".name" in localStorage) {
			name = localStorage["storage.candidates." + id + ".name"];
			region = localStorage["storage.candidates." + id + ".region"];
			party = localStorage["storage.candidates." + id + ".party"];
			votes = localStorage["storage.candidates." + id + ".votes"];
		}
